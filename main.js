...
window.doLogin = async function () {
    ...
    try {
        // ດຶງຂໍ້ມູນຈາກ Table 'Users' ບ່ອນທີ່ Email ກົງກັບທີ່ປ້ອນມາ
        const { data, error } = await supabaseClient
            .from('Users')
            .select('*')
            .eq('Email', email)
            .single(); // single() ຄືໃຫ້ດຶງມາແຄ່ແຖວດຽວ

        window.toggleLoading(false);

        if (error || !data) {
            console.error("Supabase Error:", error);
            Swal.fire('ແຈ້ງເຕືອນ', 'ບໍ່ພົບອີເມວນີ້ໃນລະບົບ', 'error');
            return;
        }

        // ກວດສອບລະຫັດຜ່ານ (ຖ້າ Password ໃນຖານຂໍ້ມູນກົງກັບທີ່ປ້ອນ)
        if (data.Password === pass) {
            ...
            // ໝາຍເຫດ: ຕອນນີ້ເຮົາປິດ window.initApp() ໄວ້ກ່ອນ ເພື່ອທົດສອບແຄ່ Login
            // ເມື່ອທຸກຢ່າງພ້ອມ ເຮົາຈຶ່ງມາເປີດໃຊ້ງານມັນຄືນ
            window.initApp();

        } else {
            Swal.fire('ແຈ້ງເຕືອນ', 'ລະຫັດຜ່ານບໍ່ຖືກຕ້ອງ', 'error');
        }
...
window.preloadDropdownDataCallback = function (resolve) {
    let promises = [
        ...
        supabaseClient.from('Organizations').select('Org_Code,Org_Name').eq('Status', 'Active').then(({ data }) => {
            activeOrgsList = (data || []).map(r => ({ id: r.Org_Code, name: r.Org_Name }));
...
window.sendToTriageFlow = async function (id, n) {
    ...
    if (result.isConfirmed) {
        ...
        else Swal.fire('ສຳເລັດ!', 'ສົ່ງເຂົ້າ Triage ແລ້ວ', 'success');
    }
};
...
window.renderEMRDrugTable = function () {
    ...
    if (currentEMRDrugs.length === 0) {
        h = '<tr><td colspan="4" class="text-center text-muted small py-2">ຍັງບໍ່ມີລາຍການ</td></tr>';
    } else {
        currentEMRDrugs.forEach((x, i) => {
            h += `<tr>
                    <td class="text-success fw-bold">${x.name}</td>
                    <td><span class="badge bg-secondary">${x.qty}</span></td>
                    <td class="small text-muted">${x.usage}</td>
                    <td class="text-center"><button type="button" class="btn btn-sm btn-danger py-0 px-2" onclick="window.removeEMRDrug(${i})"><i class="fas fa-times"></i></button></td>
                  </tr>`;
        });
    }
...
window.viewTriage = function (i) {
    ...
    let htmlContent = r.status === 'Triage' ? 'ຍັງບໍ່ວັດແທກ' : `<b>BP:</b> ${r.bp || '-'} <br> <b>Temp:</b> ${r.temp || '-'} °C <br> <b>Wt:</b> ${r.weight || '-'} kg <br> <b>Ht:</b> ${r.height || '-'} cm <br> <b>CC:</b> ${r.symptoms || '-'}`;
    Swal.fire({ title: 'Vitals', html: htmlContent, icon: 'info' });
};
...
window.printOPDCard = function (s, i) {
    ...
    google.script.run.withSuccessHandler(d => {
        ...
        try {
            ...
            safeSetText('popd_cn', d.id || "-");
            safeSetText('popd_orgid', d.org_id || d.orgId || "-");
            safeSetText('popd_orgname', d.org_name || d.orgName || "-");
            ...
            safeSetText('popd_name', d.firstname || "");
            safeSetText('popd_surname', d.lastname || "");
            safeSetText('popd_age', d.age || "-");
            safeSetText('popd_dob', d.dob || "-");
            safeSetText('popd_gender', d.gender || "-");
            safeSetText('popd_nation', d.nation || "-");
            safeSetText('popd_job', d.job || "-");
            safeSetText('popd_village', d.address || "-");
            safeSetText('popd_district', d.district || "-");
            safeSetText('popd_prov', d.province || "-");
            safeSetText('popd_phone', d.phone || "-");
            ...
            safeSetText('popd_allergy', d.allergy || "ບໍ່ມີ");
            safeSetText('popd_disease', d.disease || "ບໍ່ມີ");
        ...
        Swal.fire('ຂໍ້ຜິດພາດ', 'ບໍ່ສາມາດເຊື່ອມຕໍ່ກັບເຊີບເວີໄດ້', 'error');
    }).getPatientDetails(v.patientId);
};
...
window.submitUserForm = async function (e) {
    ...
    // In a real scenario, use Supabase Auth to register user, then add to custom Users table
    // For now we will just interact with the custom 'Users' table
    let row = {
...
window.fetchOrg = async function () {
    ...
    const { data, error } = await supabaseClient.from('Organizations').select('*').eq('Org_Code', c).single();
    if (data && !error) {
        $('#p_org_name').val(data.Org_Name);
        $('#p_discount_show').val((data.Discount_Rate || 0) + "%");
    } else {
...
window.submitOrgForm = async function (e) {
    ...
    let row = {
        Tax_ID: $('#o_cusId').val(), Contact_Person: $('#o_name').val(),
        Org_Name: $('#o_orgName').val(), Org_Code: $('#o_orgCode').val(),
        Discount_Rate: parseFloat($('#o_discount').val()) || 0
    };
...
window.handleOrgExcelUpload = function (e) {
    ...
    reader.onload = async function (evt) {
        ...
        for (let i = 1; i < jsonData.length; i++) {
            let row = jsonData[i];
            if (row.length < 1 || !row[0]) continue; // Assume Org_Code is first column
            insertData.push({
                Org_Code: row[0],
                Tax_ID: row[1] || '',
                Contact_Person: row[2] || '',
                Org_Name: row[3] || '',
                Discount_Rate: parseFloat(row[4]) || 0,
                Status: 'Active'
            });
        }
